...
class NewsCategoriesController extends Controller
{
    ...
    {
        $categories = NewsCategorories::orderBy('id', 'ASC')->orderBy('display_order', 'ASC')->get();
        return view('backend.news-categories.index',[
    ...
    public function get(Request $request){
        ...
        $out = [
            'status' => $status,
            'id' => $id,
            'news_category' => $get->news_category,
    ...
    public function store(Request $request){
        ...
        if ($validator){
            ...
            $save->slug = $req['slug'];
            $save->news_category = $req['news_category'];
    ...
    public function slugGenerator(Request $request){
        ...
        $getCount = NewsCategorories::where('slug', $slug)->count();
        if ($getCount > 0){
            $item = NewsCategorories::where('id', $id)->first();
            if (!empty($item)){
                if ($item->slug != $slug){
                    $isExist = 1;
                }
